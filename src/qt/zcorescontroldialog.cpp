...
#include "ui_zcorescontroldialog.h"
...
std::list<std::string> ZcoresControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZcoresControlDialog::listMints;

ZcoresControlDialog::ZcoresControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZcoresControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZcoresControlDialog::~ZcoresControlDialog()
{
    delete ui;
}

void ZcoresControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZcoresControlDialog::updateList()
{
...
void ZcoresControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZcoresControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZcores_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZcoresControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZcoresControlDialog::GetSelectedMints()
{
...
void ZcoresControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
