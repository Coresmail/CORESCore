                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zcors ought to be enough for anybody." - Bill Gates, 2017
    ui->zcorspayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzcorsSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzcors->setEnabled(false);
        ui->pushButtonMintzcors->setToolTip(tr("zcors is currently disabled due to maintenance."));

        ui->pushButtonSpendzcors->setEnabled(false);
        ui->pushButtonSpendzcors->setToolTip(tr("zcors is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zcorspayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzcors_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcors is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zcors...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zcors in ") +
...
void PrivacyDialog::on_pushButtonSpendzcors_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zcors is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zcors
        sendzcors();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zcors
    sendzcors();
}

void PrivacyDialog::on_pushButtonZcoresControl_clicked()
{
    ZcoresControlDialog* zcorsControl = new ZcoresControlDialog(this);
    zcorsControl->setModel(walletModel);
    zcorsControl->exec();
}

void PrivacyDialog::setZcoresControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzcorsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzcors()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid cores Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zcorspayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zcors is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " cores </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zcors</b>";
    ...
    // use mints from zcors selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZcoresControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZcoresControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcores selector in case it was used
    ZcoresControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zcors Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " cores, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zcors Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zcors </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zcors "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zcors "));
    ui->labelzcorsAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzcorsSyncStatus->setVisible(fShow);
}
