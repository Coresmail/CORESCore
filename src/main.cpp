...
#if defined(NDEBUG)
#error "cores cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 10 * 60;
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zcors spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zcors spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zcors spend with serial %s from tx %s is not in valid range\n",
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      if (nHeight == 0) {
             nSubsidy = 300000 * COIN;
                ...
                nSubsidy = 1 * COIN;
      } else if (nHeight <= 16920 && nHeight > 300) {
          nSubsidy = 5 * COIN;
      } else if (nHeight <= 41920 && nHeight > 16920) {
          nSubsidy = 10 * COIN;
      } else if (nHeight <= 61920 && nHeight > 41920) {
          nSubsidy = 50 * COIN;
      } else if (nHeight <= 101920 && nHeight > 61920) {
          nSubsidy = 100 * COIN;
      } else if (nHeight <= 151920 && nHeight > 101920) {
          nSubsidy = 400 * COIN;
      }  else if (nHeight <= 301920 && nHeight > 151920) {
          nSubsidy = 800 * COIN;
      } else if (nHeight <= 1001920 && nHeight > 301920) {
          nSubsidy = 500 * COIN;
      }  else if (nHeight <= 3001920 && nHeight > 1001920) {
          nSubsidy = 60 * COIN;
      }  else {
          nSubsidy = 5 * COIN;
      }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from cores
...
{
    RenameThread("cores-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZcoresMinted()
{
...
void RecalculateZcoresSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zcors supply
...
bool RecalculatecoresSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 91842 && pindex->GetBlockHash() == uint256("0x001")) ||
                             (pindex->nHeight == 91880 && pindex->GetBlockHash() == uint256("0x001")));
    if (fEnforceBIP30) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zcors with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zcorsSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert cores to zcors
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zcors spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zcors serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zcors mints and %d zcors spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // cores: We use certain sporks during IBD, so check to see if they are
